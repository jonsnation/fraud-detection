...
with tab1:
    ...
    with st.form("manual_form"):
        ...
        with col1:
            ...
            addr1 = st.number_input("Address 1", min_value=0, value=100)
        with col2:
            card1 = st.selectbox("Card1", [1111, 2222, 3333, 4444, 5555])
            dist1 = st.number_input("Distance", min_value=0.0, value=10.0)
        with col3:
            product = st.selectbox("Product Code", ["W", "C", "R", "H", "S"])
            email = st.selectbox("Email Domain", ["gmail.com", "yahoo.com", "hotmail.com", "anonymous.com"])
    ...
    if submitted:
        ...
        col3.metric("Ensemble Verdict", ensemble_label, f"{confidence*100:.1f}% agreement")
